...
- Vocabulary density computed two ways: gold-derived (fraction of gold/reference morphemes already in the pretrained vocab) for languages with gold segmentation (Turkish, Finnish, Icelandic, Russian, possibly Quechua), and an unsupervised estimate (Morfessor-output-vs-vocab overlap, no gold annotation needed) for the rest — reported both ways to quantify what's lost by removing the gold-annotation requirement, since the unsupervised estimate is what actually scales to the full 116-language NLLB population.
